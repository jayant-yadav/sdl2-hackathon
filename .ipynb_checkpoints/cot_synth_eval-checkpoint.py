...
MODEL_LOAD_PATH = ['../log_smhi/2023-09-29_12-06-11/model_it_100000']  # See examples on how to point to a specific model path below. None --> Model randomly initialized.
...
for model_load_path in MODEL_LOAD_PATH:  # Ensemble approach if len(MODEL_LOAD_PATH) > 1
	...
	# model = MLP6(input_dim, output_dim, apply_relu=True)
	# model = ANN_v1(input_dim, output_dim, apply_relu=True)
	# model = ANN_v2(input_dim, output_dim, apply_relu=True)
